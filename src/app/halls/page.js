...
export default function HallsPage() {
  ...
  const handleSidoChange = (e) => {
    ...
    setSigungu('전체'); // 시/도가 바뀌면 시/군/구는 '전체'로 초기화
  };
...
const SIDO_VARIATIONS = {
  ...
  const filtered = funeralHalls.filter((h) => {
    // 시/도 필터링 로직 (정확한 접두사 매칭)
    let matchSido = false;
    if (sido === '전체') {
      matchSido = true;
    } else {
    ...
    // 시/군/구 필터링 로직
    const matchSigungu = !sigungu || sigungu === '전체' || (h.address && h.address.includes(sigungu));
    
    // 키워드 검색 로직
    const matchSearch = !search || h.name.includes(search) || (h.address && h.address.includes(search));
    ...
    <div style={{ background: '#edf2f7', minHeight: '100vh', padding: '2rem 0' }}>
      <div className="container" style={{ padding: '0 1.25rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
          <select value={sido} onChange={handleSidoChange} style={{ flex: 1, minWidth: '150px', padding: '0.75rem', border: '1px solid #cbd5e0', borderRadius: '4px', background: 'white', color: '#4a5568' }}>
            {SIDO_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          <select value={sigungu} onChange={(e) => setSigungu(e.target.value)} style={{ flex: 1, minWidth: '150px', padding: '0.75rem', border: '1px solid #cbd5e0', borderRadius: '4px', background: 'white', color: '#4a5568' }}>
            {sigunguOptions.map(s => <option key={s} value={s}>{s}</option>)}
          </select>

          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} style={{ flex: 2, minWidth: '200px', padding: '0.75rem', border: '1px solid #cbd5e0', borderRadius: '4px', background: 'white' }} />
          <button style={{ background: '#4078e6', color: 'white', border: 'none', padding: '0 1.5rem', borderRadius: '4px', cursor: 'pointer', fontSize: '1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            🔍
          </button>
        </div>
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
          ...
          <div ref={scrollRef} style={{ display: 'flex', gap: '1rem', overflowX: 'auto', padding: '1rem 0', scrollSnapType: 'x mandatory', width: '100%', WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none' }}>
            {filtered.map((hall, idx) => (
              <Link key={hall.id} href={`/halls/${hall.name}`} style={{ textDecoration: 'none' }}>
                <div style={{ minWidth: '260px', height: '300px', background: 'white', borderRadius: '8px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', position: 'relative', scrollSnapAlign: 'start', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ position: 'relative', height: '180px', width: '100%', background: '#f7fafc', flexShrink: 0 }}>
                    {hall.photos && hall.photos.length > 0 ? (
                      <Image src={hall.photos[0]} alt={`${hall.name} - ${hall.regionCode === 'seoul' ? '서울' : hall.regionCode} 지역 장례식장 전경 및 빈소 안내`} fill style={{ objectFit: 'cover' }} />
                    ) : (
