...
▶ 통계·수치 인용 필수 형식:
   ...
   ※ 출처가 불확실한 수치는 수치째로 삭제하세요. 없는 데이터를 만들어내지 마세요.
...
② 검증 가능한 수치·출처 직접 명시 — 보건복지부 통계, 장사법 조항, 실비 범위 등을 인용할 때 출처를 본문 문장 안에 직접 명시하세요.
   - 올바른 형식: "보건복지부 2024년 장례 실태조사에 따르면 전국 평균 장례 비용은 약 1,370만 원입니다"
   - 절대 금지: "[cite:X]", "[cite:1]", "[출처]", "[링크]" 같은 형태의 인용 플레이스홀더
   - 출처가 불확실하면 수치 자체를 삭제하세요. 플레이스홀더를 남기는 것보다 수치를 삭제하는 것이 품질상 훨씬 낫습니다.
...
⑥ 불확실 정보·플레이스홀더 기재 절대 금지 — 아래 형태의 표현은 최종 출력물에 단 하나도 포함되어서는 안 됩니다:
   - "[정확한 주소 정보를 검색하여 기재]", "[예: OO 평]", "[확인된 편의 시설 정보 기재]" 등 대괄호([])로 감싼 플레이스홀더 표현
   - "[cite:X]", "[cite:1]", "[출처]", "[링크]", "[각주]" 등 인용 플레이스홀더 표현 — 이것도 대괄호 금지 규칙과 동일하게 적용됩니다
   - "확인 필요", "변동될 수 있음", "직접 확인 권장", "정확한 현황은 별도 확인이 필요합니다" 같은 면피성 문구
   - 이런 표현이 하나라도 포함된 글은 네이버/구글 모두에서 즉시 저품질로 분류됩니다.
   실행 원칙: 특정 장례식장·시설 글이라면 반드시 STEP 1의 googleSearch를 먼저 실행하고, 확인된 정보만 작성하세요. 검색해도 확인되지 않는 항목은 해당 항목·표·섹션 전체를 삭제하세요. 빈 셀이나 플레이스홀더가 있는 표는 표 자체를 삭제하세요.
   🚨 표(table) 작성 금지 원칙: "기본 정보", "시설 안내" 같은 표를 만들 때 셀 안에 실제 데이터(숫자, 주소, 전화번호 등) 대신 "정보 기재", "안내 예정", "노선 정보" 같은 설명문을 넣는 것은 절대 금지입니다. 실제 데이터로 채울 수 없는 표는 만들지 말고, 그 내용은 본문 문장으로 자연스럽게 녹여 쓰거나 아예 생략하세요.
...
⑧ HTML 콘텐츠 내 마크다운 문법 완전 금지 — <content> 태그 안에는 반드시 HTML 태그만 사용하세요.
   ...
   마크다운 문법을 HTML 안에 쓰면 브라우저에서 ** 기호가 그대로 노출됩니다.

⑨ 특정 장례식장 글 작성 시 검색 기반 특화 정보 필수 — 특정 병원·시설 장례식장을 주제로 쓰는 경우, STEP 1 검색을 통해 확인한 해당 시설만의 고유한 특징을 반드시 포함하세요.
   - 검색으로 확인해야 할 것: 실제 주소, 전화번호, 빈소 수, 주차 가능 대수, 인근 화장장까지 이동 시간
   - 검색으로 확인해야 할 것: 병원 유형별 특수 사항 (암병원·원자력병원→방사성 동위원소 환자 특수 절차, 어린이병원→소아 전용 절차 등)
   - 검색으로 확인해야 할 것: 인근 경쟁 장례식장과의 접근성·시설 차이
   - 검색 없이 어디에나 적용되는 범용 내용만 작성한 글은 구글·네이버 모두에서 저평가됩니다.
   - 🚨 검색 후에도 확인 불가 항목은 섹션 삭제. "문의 바랍니다", "확인하세요" 문구로 대체 금지.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[관리자 제공 데이터 활용 전략 — 글 품질의 핵심]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
AI 단독으로 100점 글을 만들 수 없는 이유는 하나입니다: Google E-E-A-T의 첫 번째 E(Experience, 직접 경험)는 AI가 증명할 수 없기 때문입니다. 이를 보완하는 방법은 두 가지입니다: 관리자가 실제 데이터를 제공하거나, AI가 웹 검색으로 실제 데이터를 직접 수집하는 것입니다.

▶ 관리자 추가 지침에 실제 시설 정보가 있을 때 (최우선 과제):
   - 제공된 주소·전화번호·빈소 수·요금·특이사항을 글에 정확히 반영하세요.
   - 검색 결과와 다를 경우 관리자 제공 정보가 우선입니다.
   - 이 데이터를 표·목록·본문에 구체적 수치로 명시할수록 글의 신뢰도가 높아집니다.

▶ 관리자 추가 지침 없이 특정 시설 글을 쓸 때:
   - STEP 1의 검색 프로토콜을 반드시 실행하여 시설 정보를 직접 수집하세요.
   - 검색으로 확인된 사실(주소, 전화번호, 시설 특성 등)을 본문에 반드시 포함하세요.
   - 검색해도 확인 불가한 항목(예: 정확한 빈소 요금)은 섹션 자체를 삭제하고, 대신 전문가 인사이트 분량을 늘리세요.
...
[🔍 STEP 1 — 글 작성 전 필수 검색 실행]
글을 쓰기 전에 googleSearch 도구를 사용하여 아래를 검색하세요:
- 키워드 "${keyword}"와 관련된 최신 정보
- 키워드에 특정 시설명(장례식장·납골당·병원 등)이 포함된 경우: 해당 시설의 주소, 전화번호, 빈소 수, 이용요금, 교통편, 특이사항을 반드시 검색하세요
- 통계 수치가 필요한 경우: 보건복지부, 통계청 등 공신력 있는 출처를 검색하여 확인된 수치만 사용하세요
- 검색 결과에서 확인된 사실만 작성하고, 확인 불가 항목은 섹션 전체를 삭제하세요
- [cite:X], [cite:1] 같은 인용 플레이스홀더는 절대 출력하지 마세요 — 출처는 문장 안에 직접 명시하거나, 불확실하면 수치째로 삭제하세요
...
`;
    ...
    // Remove any img tags pointing to local /images/ paths (Gemini occasionally generates these)
