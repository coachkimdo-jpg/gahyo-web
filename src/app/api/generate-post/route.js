...
`;
    ...
    // Strip broken img tags that point to /images/ai-posts/ since Vercel serverless cannot write files
    const stripAiImages = (html) => {
      if (!html) return html;
      return html.replace(/<img[^>]*src=["'][^"']*\/images\/ai-posts\/[^"']*["'][^>]*\/?>/gi, '');
    };

    const rawContent = cleanString(extractedContent || text);
    const finalContent = stripAiImages(rawContent);
