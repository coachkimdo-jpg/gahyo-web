...
export const metadata = {
  ...
  openGraph: {
    title: '부산 상조 | 가효상조',
    description: '부산 전지역 24시간 즉시 출동. 해운대·수영·남·동·서구 등 전 구 출동. 가입비·월납입금 0원, 장례 후 결제.',
    url: 'https://gahyo.co.kr/local/busan',
  },
};
...
const localJsonLd = {
  ...
  '@type': 'LocalBusiness',
  name: '가효상조 부산광역시',
  description: '부산광역시 24시간 즉시 출동 후불제 장례 서비스',
  url: 'https://gahyo.co.kr/local/busan',
  telephone: '1551-5718',
...
const breadcrumbJsonLd = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: '부산 상조', item: 'https://gahyo.co.kr/local/busan' },
  ],
};

const HALLS = ["부산대학교병원장례식장", "동아대학교병원장례식장", "인제대백병원장례식장", "부산의료원장례식장"];
    ...
    <>
      ...
      <main style={{ maxWidth: '860px', margin: '0 auto', padding: '3rem 1.25rem 5rem' }}>
        <nav aria-label="breadcrumb" style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', marginBottom: '1.5rem' }}>
          <Link href="/">홈</Link> › <Link href="/local">지역별 서비스</Link> › 부산 상조
        </nav>
        ...
        <p style={{ fontSize: '1.05rem', color: 'var(--text-secondary)', lineHeight: 1.8, marginBottom: '2rem' }}>
          부산 16개 구·군 전역에 24시간 즉시 출동합니다. 해운대구, 수영구, 남구, 동구, 서구, 부산진구 등 어디서나 임종 직후 연락 주시면 즉시 대응합니다. <strong>가입비·월납입금 0원, 장례 종료 후 결제</strong>하는 완전 후불제 상조입니다.
        </p>
        <section style={{ background: 'var(--navy-light)', borderRadius: '16px', padding: '2rem', marginBottom: '2.5rem' }}>
          <h2 style={{ fontSize: '1.1rem', fontWeight: '700', marginBottom: '1rem', color: 'var(--navy)' }}>부산광역시 출동 서비스 안내</h2>
          <ul style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem', listStyle: 'none', margin: 0, padding: 0 }}>
            {[['출동 범위','부산 16개 구·군'],['출동 시간','24시간 · 연중무휴'],['가입비','0원'],['월 납입금','0원'],['결제 방식','장례 종료 후 100% 후불'],['담당 지도사','국가공인 1급 1:1 전담']].map(([k, v]) => (
        ...
        <section style={{ marginBottom: '2.5rem' }}>
          <h2 style={{ fontSize: '1.1rem', fontWeight: '700', color: 'var(--text-primary)', marginBottom: '1rem' }}>부산광역시 주요 제휴 장례식장</h2>
        ...
        <section style={{ background: 'var(--navy)', borderRadius: '16px', padding: '2.5rem', textAlign: 'center', color: 'white' }}>
          <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.5rem', fontWeight: '700', marginBottom: '0.5rem' }}>부산광역시 장례, 지금 바로 연락하세요</h2>
