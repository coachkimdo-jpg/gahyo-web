...
// 봉안시설 - ossuaries.json (type='봉안당' 고정)
...
// 자연장지 - naturalBurials.json (type='수목장' 고정)
...
export const CEMETERY_SIDO_VARIATIONS = {
  "서울특별시": ["서울"],
  "부산광역시": ["부산"],
  "대구광역시": ["대구"],
  "인천광역시": ["인천"],
...
// cemeteries 목록 필터링 (API route와 서버 컴포넌트 SSR 초기 렌더가 동일 로직을 공유)
export function filterCemeteries({ type = '', search = '', sido = '전체', sigungu = '전체' } = {}) {
  const filtered = combinedData.filter((c) => {
    ...
    if (sido === '전체') {
      ...
      const variations = CEMETERY_SIDO_VARIATIONS[sido] || [sido];
